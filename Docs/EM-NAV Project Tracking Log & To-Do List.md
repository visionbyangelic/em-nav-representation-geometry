...
* \[X] Run the self-contained baseline execute\_stage\_zero\_scan(maze\_size=12) script.  
* \[X] **Log Scan Outputs:**  
...
![EM-NAV Phase 0 Scan Output](https://raw.githubusercontent.com/visionbyangelic/em-nav-representation-geometry/main/Docs/outputpng/phase0.png)
...
* \[X] **Agent D (Recurrent SNN / RSNN):** Construct fully recurrent spiking network integrating hidden LIF loops and a strict $L\_1$ activity regularization penalty.
...
* \[X]**Task 2 (Intrinsic Curiosity Coverage):** Configure space-filling novelty optimization track utilizing space-occupancy tracking metrics ($R\_t \= 1 / \\sqrt{N(x, y)}$).  

![EM-NAV Phase 1 Output](\outputpng\phase1.png)


* \[ \] Deploy optimization runs across 3 independent random seeds per configuration (24 unique training runs, $1 \\times 10^6$ steps each).  
* \[ \] Lock and save optimized network checkpoint configuration files (.pt format).

## ---

**Phase 2 & 3: Content and Geometric Characterization**

* \[ \] Permanently freeze all network synaptic weights following the completion of the training blocks.  
* \[ \] Execute 5,000-step pure evaluation runs to harvest activation vectors.  
* \[ \] **Stage 2 (Linear Probing Content Check):** Train an un-tuned linear ridge regression probe to decode absolute continuous $(x, y)$ coordinate sets. Evaluate Mean Squared Error (MSE).  
* \[ \] **Stage 3 (Geometric Manifold Alignment Mapping via Tri-RSA):**  
  * \[ \] Compute population-level Representational Dissimilarity Matrices (RDMs) using correlation distances ($1 \- \\text{Pearson's } r$).  
  * \[ \] Construct competitive hypothesis models (Sensorimotor Proximity, flat Euclidean, and Navigable Geodesic path layouts).  
  * \[ \] Run partial rank correlation profiles (Kendall's $\\tau$) across all four architectures to determine structural alignment.

## ---

**Phase 4 & 5: Single-Unit Metrics & Scientific Decision Gate**

* \[ \] **Stage 4 (Single-Unit Firing Analysis):** Generate standard 2D spatial firing rate heatmaps for each of the 32 hidden neurons across all architectures.  
* \[ \] Score spatial tuning density profiles using the Skaggs Spatial Information Index ($I$).  
* \[ \] Execute 1,000-iteration temporal time-shift shuffle null controls to isolate true place cell profiles (\>95th percentile).  
...
* \[ \] Bind the 5-ray physical proximity cast pipeline using native scene.ray\_cast momentum logic.  
